...
class Litchi_Vendor {
    ...
    public function is_vendor() {
        return litchi_is_user_seller( $this->id );
    }
    ...
    public function is_enabled() {
        return litchi_is_seller_enabled( $this->id );
    }
    ...
    public function is_trusted() {
        return litchi_is_seller_trusted( $this->id );
    }
    ...
    public function is_featured() {
        return 'yes' == get_user_meta( $this->id, 'litchi_feature_seller', true );
    }
    ...
    public function get_shop_url() {
        return litchi_get_store_url( $this->id );
    }
    ...
    public function get_total_sales() {
        return litchi_author_total_sales( $this->id );
    }
    ...
    public function get_product_views() {
        return (int) litchi_author_pageviews( $this->id );
    }
    ...
    public function get_earnings( $formatted = true, $on_date = '' ) {
        ...
        $status        = litchi_withdraw_get_active_order_status_in_comma();
        $cache_group   = 'litchi_seller_data_'.$this->id;
        $cache_key     = 'litchi_seller_earnings_' . $this->id;
        $earning       = wp_cache_get( $cache_key, $cache_group );
        $on_date       = $on_date ? date( 'Y-m-d', strtotime( $on_date ) ) : current_time( 'mysql' );
        $trn_type      = 'litchi_refund';
        $refund_status = 'approved';

        if ( false === $earning ) {
            $installed_version = get_option( 'litchi_theme_version' );
            ...
            if ( ! $installed_version || version_compare( $installed_version, '2.8.2', '>' ) ) {
                $debit_balance  = $wpdb->get_row( $wpdb->prepare(
                    ...
                    FROM {$wpdb->prefix}litchi_vendor_balance
                    WHERE
               ...
               $credit_balance = $wpdb->get_row( $wpdb->prepare(
                ...
                $result = $wpdb->get_row( $wpdb->prepare(
                    ...
                    FROM
                        {$wpdb->prefix}litchi_orders as do LEFT JOIN {$wpdb->prefix}posts as p ON do.order_id = p.ID
                    WHERE
            ...
            litchi_cache_update_group( $cache_key , $cache_group );
        }

        if ( $formatted ) {
            return apply_filters( 'litchi_get_formatted_seller_earnings', wc_price( $earning ), $this->id );
        }

        return apply_filters( 'litchi_get_seller_earnings', $earning, $this->id );
    }
    ...
    public function get_balance( $formatted = true, $on_date= '' ) {
        ...
        $status        = litchi_withdraw_get_active_order_status_in_comma();
        $cache_group   = 'litchi_seller_data_'.$this->id;
        $cache_key     = 'litchi_seller_balance_' . $this->id;
        $earning       = wp_cache_get( $cache_key, $cache_group );
        $threshold_day = litchi_get_option( 'withdraw_date_limit', 'litchi_withdraw', 0 );
        $on_date       = $on_date ? date( 'Y-m-d', strtotime( $on_date ) ) : current_time( 'mysql' );
        ...
        if ( false === $earning ) {
            ...
            if ( ! $installed_version || version_compare( $installed_version, '2.8.2', '>' ) ) {
                ...
                //     (SELECT SUM(credit) FROM {$wpdb->prefix}litchi_vendor_balance WHERE vendor_id = %d AND DATE(balance_date) <= %s) as withdraw
                //     from {$wpdb->prefix}litchi_vendor_balance
                ...
                //     (SELECT SUM(amount) FROM {$wpdb->prefix}litchi_withdraw WHERE user_id = %d AND status = 1 AND DATE(`date`) <= %s) as withdraw
                //     FROM {$wpdb->prefix}litchi_orders as do LEFT JOIN {$wpdb->prefix}posts as p ON do.order_id = p.ID
        ...
        if ( $formatted ) {
            return apply_filters( 'litchi_get_formatted_seller_balance', wc_price( $earning ) );
        }

        return apply_filters( 'litchi_get_seller_balance', $earning );
    }
    ...
    public function get_rating() {
        ...
        $rating_value = apply_filters( 'litchi_seller_rating_value', array(
    ...
    public function get_readable_rating( $display = true ) {
        ...
        if ( ! $rating['count'] ) {
            $html = __( 'No ratings found yet!', 'litchi-lite' );
        } else {
            $long_text   = _n( '%s rating from %d review', '%s rating from %d reviews', $rating['count'], 'litchi-lite' );
            $text        = sprintf( __( 'Rated %s out of %d', 'litchi-lite' ), $rating['rating'], number_format( 5 ) );
            $width       = ( $rating['rating']/5 ) * 100;
            $review_text = sprintf( $long_text, $rating['rating'], $rating['count'] );

            if ( function_exists( 'litchi_get_review_url' ) ) {
                $review_text = sprintf( '<a href="%s">%s</a>', esc_url( litchi_get_review_url( $this->id ) ), $review_text );
            }
    ...
    public function get_percentage( $product_id = 0 ) {
        return litchi_get_seller_percentage( $this->id, $product_id );
    }
    ...
    public function make_active() {
        update_user_meta( $this->get_id(), 'litchi_enable_selling', 'yes' );
        $this->change_product_status( 'publish' );

        do_action( 'litchi_vendor_enabled', $this->get_id() );
    ...
    public function make_inactive() {
        update_user_meta( $this->get_id(), 'litchi_enable_selling', 'no' );
        $this->change_product_status( 'pending' );

        do_action( 'litchi_vendor_disabled', $this->get_id() );
