...
/**
* Base REST Controller for litchi
*
* @since 2.8.0
*
* @package litchi
...
abstract class Litchi_REST_Controller extends WP_REST_Controller {
    ...
    public function get_item( $request ) {
        ...
        if ( ! empty( $post->post_type ) && 'product_variation' === $post->post_type && 'product' === $this->post_type ) {
            return new WP_Error( "litchi_rest_invalid_{$this->post_type}_id", __( 'To manipulate product variations you should use the /products/&lt;product_id&gt;/variations/&lt;id&gt; endpoint.', 'litchi' ), array( 'status' => 404 ) );
        } elseif ( empty( $id ) || empty( $post->ID ) || $post->post_type !== $this->post_type ) {
            return new WP_Error( "litchi_rest_invalid_{$this->post_type}_id", __( 'Invalid ID.', 'litchi' ), array( 'status' => 404 ) );
        }
    ...
    public function create_item( $request ) {
        ...
        try {
            ...
            //Update post author
            wp_update_post( array( 'ID' => $object->get_id(), 'post_author' => litchi_get_current_user_id() ) );
    ...
    public function delete_item( $request ) {
        ...
        if ( ! $result ) {
            return new WP_Error( 'litchi_rest_cannot_delete', sprintf( __( 'The %s cannot be deleted.', 'litchi' ), $this->post_type ), array( 'status' => 500 ) );
        }

        do_action( "litchi_rest_delete_{$this->post_type}_object", $object, $response, $request );
     ...
     *
     * @since  2.8.0
     * @param  Litchi_Data         $object  Object data.
    ...
    protected function prepare_objects_query( $request ) {
        ...
        $args['author']              = !isset( $request['id'] ) ? litchi_get_current_user_id() : $request['id'];
