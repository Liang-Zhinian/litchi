...
* @author 
*/
class Litchi_REST_Store_Controller extends WP_REST_Controller {
    ...
    public function get_stores( $request ) {
        ...
        $args = apply_filters( 'litchi_rest_get_stores_args', $args, $request );

        $stores       = litchi()->vendor->get_vendors( $args );
        $data_objects = array();
        ...
        $response = $this->format_collection_response( $response, $request, litchi()->vendor->get_total() );
    ...
    public function get_store( $request ) {
        ...
        $store = litchi()->vendor->get( $store_id );

        if ( empty( $store->id ) ) {
            return new WP_Error( 'no_store_found', __( 'No store found', 'litchi' ), array( 'status' => 404 ) );
        }
    ...
    public function delete_store( $request ) {
        ...
        if ( empty( $store_id ) ) {
            return new WP_Error( 'no_vendor_found', __( 'No vendor found for updating status', 'litchi' ), array( 'status' => 400 ) );
        }

        if ( ! empty( $reassign ) ) {
            if ( $reassign === $store_id || ! get_userdata( $reassign ) ) {
                return new WP_Error( 'rest_user_invalid_reassign', __( 'Invalid user ID for reassignment.', 'litchi' ), array( 'status' => 400 ) );
            }
        }

        $vendor = litchi()->vendor->get( intval( $store_id ) )->delete( $reassign );
    ...
    public function update_store( $request ) {
        ...
        do_action( 'litchi_rest_stores_update_store', $store, $request );
    ...
    public function format_collection_response( $response, $request, $total_items ) {
        ...
        if ( function_exists( 'litchi_get_seller_status_count' ) && current_user_can( 'manage_options' ) ) {
            $counts = litchi_get_seller_status_count();
    ...
    public function get_store_products( $request ) {
        $product_controller = new Litchi_REST_Product_Controller();
    ...
    public function get_store_reviews( $request ) {
        ...
        if ( class_exists( 'Litchi_Store_Reviews' ) ) {
            $args = array(
                'post_type'      => 'litchi_store_reviews',
                'meta_key'       => 'store_id',
                'meta_value'     => $store_id,
                'post_status'    => 'publish',
            ...
            if ( empty( $query->posts ) ) {
                return new WP_Error( 'no_reviews_found', __( 'No reviews found', 'litchi' ), array( 'status' => 404 ) );
            }
            ...
            $litchi_template_reviews = Litchi_Pro_Reviews::init();
            $post_type              = 'product';
            $limit                  = (int) $params['per_page'];
            $paged                  = (int) ( $params['page'] - 1 ) * $params['per_page'];
            $status                 = '1';
            $comments               = $litchi_template_reviews->comment_query( $store_id, $post_type, $limit, $status, $paged );
    ...
    public function prepare_item_for_response( $store, $request, $additional_fields = [] ) {
        ...
        $data = $store->to_array();
        $data = array_merge( $data, apply_filters( 'litchi_rest_store_additional_fields', $additional_fields, $store, $request ) );
        $response = rest_ensure_response( $data );
        $response->add_links( $this->prepare_links( $data, $request ) );

        return apply_filters( 'litchi_rest_prepare_store_item_for_response', $response );
    }
