...
class Litchi_REST_Product_Controller extends Litchi_REST_Controller {
    ...
    public function register_routes() {
        ...
        register_rest_route( $this->namespace, '/' . $this->base . '/(?P<id>[\d]+)/related', array(
            'args' => array(
                ...
                'number' => array(
                    'description' => __( 'Number of product you want to get top rated product', 'litchi' ),
                    'type'        => 'integer',
                    'default'     => 10
                )
            ),
        ...
        register_rest_route( $this->namespace, '/' . $this->base . '/top_rated', array(
            'args' => array(
                ...
                'seller_id' => array(
                    'description' => __( 'Top rated product for specific vendor', 'litchi' ),
                    'type'        => 'integer',
                ),
            ),
            array(
    ...
    public function validation_before_create_item( $request ) {
        $store_id = litchi_get_current_user_id();

        if ( empty( $store_id ) ) {
            return new WP_Error( 'no_store_found', __( 'No seller found', 'litchi' ), array( 'status' => 404 ) );
        }

        if ( ! empty( $request['id'] ) ) {
            return new WP_Error( "woocommerce_rest_{$this->post_type}_exists", sprintf( __( 'Cannot create existing %s.', 'litchi' ), 'product' ), array( 'status' => 400 ) );
        }

        if ( empty( $request['name'] ) ) {
            return new WP_Error( "litchi_product_no_title_found", sprintf( __( 'Product title must be required', 'litchi' ), 'product' ), array( 'status' => 404 ) );
        }

        $category_selection = litchi_get_option( 'product_category_style', 'litchi_selling', 'single' );

        if ( empty( $request['categories'] ) ) {
            return new WP_Error( "litchi_product_category", __( 'Category must be required', 'litchi' ), array( 'status' => 404 ) );
        }

        if (  'single' == $category_selection ) {
            if ( count( $request['categories'] ) > 1  ) {
                return new WP_Error( "litchi_product_category_no_more_one", __( 'You can not select more than category', 'litchi' ), array( 'status' => 404 ) );
            }
        }
    ...
    public function validation_before_update_item( $request ) {
        ...
        if ( ! $object || 0 === $object->get_id() ) {
            return new WP_Error( "litchi_rest_{$this->post_type}_invalid_id", __( 'Invalid ID.', 'litchi' ), array( 'status' => 400 ) );
        }

        $product_author = get_post_field( 'post_author', $object->get_id() );

        if ( $store_id != $product_author ) {
            return new WP_Error( "litchi_rest_{$this->post_type}_invalid_id", __( 'Sorry, you have no permission to do this. Since it\'s not your product.', 'litchi' ), array( 'status' => 400 ) );
        }
    ...
    public function validation_before_delete_item( $request ) {
        ...
        if ( ! $object || 0 === $object->get_id() ) {
            return new WP_Error( "litchi_rest_{$this->post_type}_invalid_id", __( 'Invalid ID.', 'litchi' ), array( 'status' => 404 ) );
        }
    ...
    public function get_product_permissions_check() {
        return current_user_can( 'litchi_view_product_menu' );
    }
    ...
    public function create_product_permissions_check() {
        return current_user_can( 'litchi_add_product' );
    }
    ...
    public function get_single_product_permissions_check() {
        return current_user_can( 'litchidar' );
    }
    ...
    public function update_product_permissions_check() {
        return current_user_can( 'litchi_edit_product' );
    }
    ...
    public function delete_product_permissions_check() {
        return current_user_can( 'litchi_delete_product' );
    }
    ...
    public function get_product_summary_permissions_check() {
        return current_user_can( 'litchi_view_product_status_report' );
    }
    ...
    public function get_product_summary( $request ) {
        $seller_id = litchi_get_current_user_id();

        $data = array(
            'post_counts'  => litchi_count_posts( 'product', $seller_id ),
            'products_url' => litchi_get_navigation_url( 'products' ),
        );
    ...
    public function get_top_rated_product( $request ) {
        $result   = litchi_get_top_rated_products( $request['number'], $request['seller_id'] );
        $data     = array();
        $response = array();
    ...
    public function get_best_selling_product( $request ) {
        $result   = litchi_get_best_selling_products( $request['number'], $request['seller_id'] );
        $data     = array();
        $response = array();
    ...
    public function get_featured_product( $request ) {
        $result   = litchi_get_featured_products( $request['number'], $request['seller_id'] );
        $data     = array();
        $response = array();
    ...
    public function get_latest_product( $request ) {
        $result   = litchi_get_latest_products( $request['number'], $request['seller_id'] );
        $data     = array();
        $response = array();
    ...
    protected function prepare_data_for_response( $product, $request ) {
        ...
        return apply_filters( "litchi_rest_prepare_{$this->post_type}_object", $response, $product, $request );
    }
    ...
    protected function prepare_object_for_database( $request, $creating = false ) {
        ...
        if ( 'variation' === $product->get_type() ) {
            return new WP_Error( "litchi_rest_invalid_{$this->post_type}_id", __( 'To manipulate product variations you should use the /products/&lt;product_id&gt;/variations/&lt;id&gt; endpoint.', 'litchi' ), array(
                'status' => 404,
            ) );
        }
        ...
        // Post status.
        if ( litchi_is_seller_trusted( get_current_user_id() ) ) {
            if ( isset( $request['status'] ) ) {
                $product->set_status( get_post_status_object( $request['status'] ) ? $request['status'] : 'draft' );
            }
        } else {
            $status = litchi_get_option( 'product_status', 'litchi_selling', 'pending' );
    ...
    protected function get_images( $product ) {
        ...
        if ( empty( $images ) ) {
            $images[] = array(
                ...
                'name'              => __( 'Placeholder', 'litchi' ),
                'alt'               => __( 'Placeholder', 'litchi' ),
                'position'          => 0,
            );
        }
        ...
        if ( ! empty( $images ) ) {
            ...
            foreach ( $images as $image ) {
                ...
                if ( ! wp_attachment_is_image( $attachment_id ) ) {
                    ...
                    throw new WC_REST_Exception( 'woocommerce_product_invalid_image_id', sprintf( __( '#%s is an invalid image ID.', 'litchi' ), $attachment_id ), 400 );
                }
    ...
    public function get_item_schema() {
        ...
        $schema         = array(
            ...
            'properties' => array(
                'id'                    => array(
                    'description' => __( 'Unique identifier for the resource.', 'litchi' ),
                    'type'        => 'integer',
                    'context'     => array( 'view', 'edit' ),
                    'readonly'    => true,
                ),
                'name'                  => array(
                    'description' => __( 'Product name.', 'litchi' ),
                    'type'        => 'string',
                    'context'     => array( 'view', 'edit' ),
                ),
                'slug'                  => array(
                    'description' => __( 'Product slug.', 'litchi' ),
                    'type'        => 'string',
                    'context'     => array( 'view', 'edit' ),
                ),
                'permalink'             => array(
                    'description' => __( 'Product URL.', 'litchi' ),
                    'type'        => 'string',
                    'format'      => 'uri',
                    'context'     => array( 'view', 'edit' ),
                    'readonly'    => true,
                ...
                'date_created'          => array(
                    'description' => __( "The date the product was created, in the site's timezone.", 'litchi' ),
                    'type'        => 'date-time',
                    'context'     => array( 'view', 'edit' ),
                    'readonly'    => true,
                ),
                'date_created_gmt'      => array(
                    'description' => __( 'The date the product was created, as GMT.', 'litchi' ),
                    'type'        => 'date-time',
                    'context'     => array( 'view', 'edit' ),
                    'readonly'    => true,
                ),
                'date_modified'         => array(
                    'description' => __( "The date the product was last modified, in the site's timezone.", 'litchi' ),
                    'type'        => 'date-time',
                    'context'     => array( 'view', 'edit' ),
                    'readonly'    => true,
                ),
                'date_modified_gmt'     => array(
                    'description' => __( 'The date the product was last modified, as GMT.', 'litchi' ),
                    'type'        => 'date-time',
                    'context'     => array( 'view', 'edit' ),
                    'readonly'    => true,
                ),
                'type'                  => array(
                    'description' => __( 'Product type.', 'litchi' ),
                    'type'        => 'string',
                    'default'     => 'simple',
                ...
                'status'                => array(
                    'description' => __( 'Product status (post status).', 'litchi' ),
                    'type'        => 'string',
                    'default'     => 'publish',
                ...
                'featured'              => array(
                    'description' => __( 'Featured product.', 'litchi' ),
                    'type'        => 'boolean',
                    'default'     => false,
                    'context'     => array( 'view', 'edit' ),
                ),
                'catalog_visibility'    => array(
                    'description' => __( 'Catalog visibility.', 'litchi' ),
                    'type'        => 'string',
                    'default'     => 'visible',
                ...
                'description'           => array(
                    'description' => __( 'Product description.', 'litchi' ),
                    'type'        => 'string',
                    'context'     => array( 'view', 'edit' ),
                ),
                'short_description'     => array(
                    'description' => __( 'Product short description.', 'litchi' ),
                    'type'        => 'string',
                    'context'     => array( 'view', 'edit' ),
                ),
                'sku'                   => array(
                    'description' => __( 'Unique identifier.', 'litchi' ),
                    'type'        => 'string',
                    'context'     => array( 'view', 'edit' ),
                ),
                'price'                 => array(
                    'description' => __( 'Current product price.', 'litchi' ),
                    'type'        => 'string',
                    'context'     => array( 'view', 'edit' ),
                    'readonly'    => true,
                ),
                'regular_price'         => array(
                    'description' => __( 'Product regular price.', 'litchi' ),
                    'type'        => 'string',
                    'context'     => array( 'view', 'edit' ),
                ),
                'sale_price'            => array(
                    'description' => __( 'Product sale price.', 'litchi' ),
                    'type'        => 'string',
                    'context'     => array( 'view', 'edit' ),
                ),
                'date_on_sale_from'     => array(
                    'description' => __( "Start date of sale price, in the site's timezone.", 'litchi' ),
                    'type'        => 'date-time',
                    'context'     => array( 'view', 'edit' ),
                ),
                'date_on_sale_from_gmt' => array(
                    'description' => __( 'Start date of sale price, as GMT.', 'litchi' ),
                    'type'        => 'date-time',
                    'context'     => array( 'view', 'edit' ),
                ),
                'date_on_sale_to'       => array(
                    'description' => __( "End date of sale price, in the site's timezone.", 'litchi' ),
                    'type'        => 'date-time',
                    'context'     => array( 'view', 'edit' ),
                ),
                'date_on_sale_to_gmt'   => array(
                    'description' => __( 'End date of sale price, as GMT.', 'litchi' ),
                    'type'        => 'date-time',
                    'context'     => array( 'view', 'edit' ),
                ),
                'price_html'            => array(
                    'description' => __( 'Price formatted in HTML.', 'litchi' ),
                    'type'        => 'string',
                    'context'     => array( 'view', 'edit' ),
                    'readonly'    => true,
                ),
                'on_sale'               => array(
                    'description' => __( 'Shows if the product is on sale.', 'litchi' ),
                    'type'        => 'boolean',
                    'context'     => array( 'view', 'edit' ),
                    'readonly'    => true,
                ),
                'purchasable'           => array(
                    'description' => __( 'Shows if the product can be bought.', 'litchi' ),
                    'type'        => 'boolean',
                    'context'     => array( 'view', 'edit' ),
                    'readonly'    => true,
                ),
                'total_sales'           => array(
                    'description' => __( 'Amount of sales.', 'litchi' ),
                    'type'        => 'integer',
                    'context'     => array( 'view', 'edit' ),
                    'readonly'    => true,
                ),
                'virtual'               => array(
                    'description' => __( 'If the product is virtual.', 'litchi' ),
                    'type'        => 'boolean',
                    'default'     => false,
                    'context'     => array( 'view', 'edit' ),
                ),
                'downloadable'          => array(
                    'description' => __( 'If the product is downloadable.', 'litchi' ),
                    'type'        => 'boolean',
                    'default'     => false,
                    'context'     => array( 'view', 'edit' ),
                ),
                'downloads'             => array(
                    'description' => __( 'List of downloadable files.', 'litchi' ),
                    'type'        => 'array',
                    'context'     => array( 'view', 'edit' ),
                    'items'       => array(
                        'type'       => 'object',
                        'properties' => array(
                            'id'   => array(
                                'description' => __( 'File MD5 hash.', 'litchi' ),
                                'type'        => 'string',
                                'context'     => array( 'view', 'edit' ),
                                'readonly'    => true,
                            ),
                            'name' => array(
                                'description' => __( 'File name.', 'litchi' ),
                                'type'        => 'string',
                                'context'     => array( 'view', 'edit' ),
                            ),
                            'file' => array(
                                'description' => __( 'File URL.', 'litchi' ),
                                'type'        => 'string',
                                'context'     => array( 'view', 'edit' ),
                            ),
                        ),
                ...
                'download_limit'        => array(
                    'description' => __( 'Number of times downloadable files can be downloaded after purchase.', 'litchi' ),
                    'type'        => 'integer',
                    'default'     => -1,
                    'context'     => array( 'view', 'edit' ),
                ),
                'download_expiry'       => array(
                    'description' => __( 'Number of days until access to downloadable files expires.', 'litchi' ),
                    'type'        => 'integer',
                    'default'     => -1,
                    'context'     => array( 'view', 'edit' ),
                ),
                'external_url'          => array(
                    'description' => __( 'Product external URL. Only for external products.', 'litchi' ),
                    'type'        => 'string',
                    'format'      => 'uri',
                    'context'     => array( 'view', 'edit' ),
                ),
                'button_text'           => array(
                    'description' => __( 'Product external button text. Only for external products.', 'litchi' ),
                    'type'        => 'string',
                    'context'     => array( 'view', 'edit' ),
                ),
                'tax_status'            => array(
                    'description' => __( 'Tax status.', 'litchi' ),
                    'type'        => 'string',
                    'default'     => 'taxable',
                ...
                'tax_class'             => array(
                    'description' => __( 'Tax class.', 'litchi' ),
                    'type'        => 'string',
                    'context'     => array( 'view', 'edit' ),
                ),
                'manage_stock'          => array(
                    'description' => __( 'Stock management at product level.', 'litchi' ),
                    'type'        => 'boolean',
                    'default'     => false,
                    'context'     => array( 'view', 'edit' ),
                ),
                'stock_quantity'        => array(
                    'description' => __( 'Stock quantity.', 'litchi' ),
                    'type'        => 'integer',
                    'context'     => array( 'view', 'edit' ),
                ),
                'in_stock'              => array(
                    'description' => __( 'Controls whether or not the product is listed as "in stock" or "out of stock" on the frontend.', 'litchi' ),
                    'type'        => 'boolean',
                    'default'     => true,
                    'context'     => array( 'view', 'edit' ),
                ),
                'backorders'            => array(
                    'description' => __( 'If managing stock, this controls if backorders are allowed.', 'litchi' ),
                    'type'        => 'string',
                    'default'     => 'no',
                ...
                'backorders_allowed'    => array(
                    'description' => __( 'Shows if backorders are allowed.', 'litchi' ),
                    'type'        => 'boolean',
                    'context'     => array( 'view', 'edit' ),
                    'readonly'    => true,
                ),
                'backordered'           => array(
                    'description' => __( 'Shows if the product is on backordered.', 'litchi' ),
                    'type'        => 'boolean',
                    'context'     => array( 'view', 'edit' ),
                    'readonly'    => true,
                ),
                'sold_individually'     => array(
                    'description' => __( 'Allow one item to be bought in a single order.', 'litchi' ),
                    'type'        => 'boolean',
                    'default'     => false,
                    'context'     => array( 'view', 'edit' ),
                ),
                'weight'                => array(
                    ...
                    'description' => sprintf( __( 'Product weight (%s).', 'litchi' ), $weight_unit ),
                    'type'        => 'string',
                    'context'     => array( 'view', 'edit' ),
                ),
                'dimensions'            => array(
                    'description' => __( 'Product dimensions.', 'litchi' ),
                    'type'        => 'object',
                    'context'     => array( 'view', 'edit' ),
                    'properties'  => array(
                        'length' => array(
                            /* translators: %s: dimension unit */
                            'description' => sprintf( __( 'Product length (%s).', 'litchi' ), $dimension_unit ),
                            'type'        => 'string',
                            'context'     => array( 'view', 'edit' ),
                        ),
                        'width'  => array(
                            /* translators: %s: dimension unit */
                            'description' => sprintf( __( 'Product width (%s).', 'litchi' ), $dimension_unit ),
                            'type'        => 'string',
                            'context'     => array( 'view', 'edit' ),
                        ),
                        'height' => array(
                            /* translators: %s: dimension unit */
                            'description' => sprintf( __( 'Product height (%s).', 'litchi' ), $dimension_unit ),
                            'type'        => 'string',
                            'context'     => array( 'view', 'edit' ),
                        ),
                    ),
                ...
                'shipping_required'     => array(
                    'description' => __( 'Shows if the product need to be shipped.', 'litchi' ),
                    'type'        => 'boolean',
                    'context'     => array( 'view', 'edit' ),
                    'readonly'    => true,
                ),
                'shipping_taxable'      => array(
                    'description' => __( 'Shows whether or not the product shipping is taxable.', 'litchi' ),
                    'type'        => 'boolean',
                    'context'     => array( 'view', 'edit' ),
                    'readonly'    => true,
                ),
                'shipping_class'        => array(
                    'description' => __( 'Shipping class slug.', 'litchi' ),
                    'type'        => 'string',
                    'context'     => array( 'view', 'edit' ),
                ),
                'shipping_class_id'     => array(
                    'description' => __( 'Shipping class ID.', 'litchi' ),
                    'type'        => 'integer',
                    'context'     => array( 'view', 'edit' ),
                    'readonly'    => true,
                ),
                'reviews_allowed'       => array(
                    'description' => __( 'Allow reviews.', 'litchi' ),
                    'type'        => 'boolean',
                    'default'     => true,
                    'context'     => array( 'view', 'edit' ),
                ),
                'average_rating'        => array(
                    'description' => __( 'Reviews average rating.', 'litchi' ),
                    'type'        => 'string',
                    'context'     => array( 'view', 'edit' ),
                    'readonly'    => true,
                ),
                'rating_count'          => array(
                    'description' => __( 'Amount of reviews that the product have.', 'litchi' ),
                    'type'        => 'integer',
                    'context'     => array( 'view', 'edit' ),
                    'readonly'    => true,
                ),
                'related_ids'           => array(
                    'description' => __( 'List of related products IDs.', 'litchi' ),
                    'type'        => 'array',
                    'items'       => array(
                        'type' => 'integer',
                    ),
                ...
                'upsell_ids'            => array(
                    'description' => __( 'List of up-sell products IDs.', 'litchi' ),
                    'type'        => 'array',
                    'items'       => array(
                        'type' => 'integer',
                    ),
                ...
                'cross_sell_ids'        => array(
                    'description' => __( 'List of cross-sell products IDs.', 'litchi' ),
                    'type'        => 'array',
                    'items'       => array(
                        'type' => 'integer',
                    ),
                ...
                'parent_id'             => array(
                    'description' => __( 'Product parent ID.', 'litchi' ),
                    'type'        => 'integer',
                    'context'     => array( 'view', 'edit' ),
                ),
                'purchase_note'         => array(
                    'description' => __( 'Optional note to send the customer after purchase.', 'litchi' ),
                    'type'        => 'string',
                    'context'     => array( 'view', 'edit' ),
                ),
                'categories'            => array(
                    'description' => __( 'List of categories.', 'litchi' ),
                    'type'        => 'array',
                    'context'     => array( 'view', 'edit' ),
                    'items'       => array(
                        'type'       => 'object',
                        'properties' => array(
                            'id'   => array(
                                'description' => __( 'Category ID.', 'litchi' ),
                                'type'        => 'integer',
                                'context'     => array( 'view', 'edit' ),
                            ),
                            'name' => array(
                                'description' => __( 'Category name.', 'litchi' ),
                                'type'        => 'string',
                                'context'     => array( 'view', 'edit' ),
                                'readonly'    => true,
                            ),
                            'slug' => array(
                                'description' => __( 'Category slug.', 'litchi' ),
                                'type'        => 'string',
                                'context'     => array( 'view', 'edit' ),
                                'readonly'    => true,
                            ),
                ...
                'tags'                  => array(
                    'description' => __( 'List of tags.', 'litchi' ),
                    'type'        => 'array',
                    'context'     => array( 'view', 'edit' ),
                    'items'       => array(
                        'type'       => 'object',
                        'properties' => array(
                            'id'   => array(
                                'description' => __( 'Tag ID.', 'litchi' ),
                                'type'        => 'integer',
                                'context'     => array( 'view', 'edit' ),
                            ),
                            'name' => array(
                                'description' => __( 'Tag name.', 'litchi' ),
                                'type'        => 'string',
                                'context'     => array( 'view', 'edit' ),
                                'readonly'    => true,
                            ),
                            'slug' => array(
                                'description' => __( 'Tag slug.', 'litchi' ),
                                'type'        => 'string',
                                'context'     => array( 'view', 'edit' ),
                                'readonly'    => true,
                            ),
                ...
                'images'                => array(
                    'description' => __( 'List of images.', 'litchi' ),
                    'type'        => 'array',
                    'context'     => array( 'view', 'edit' ),
                    'items'       => array(
                        'type'       => 'object',
                        'properties' => array(
                            'id'                => array(
                                'description' => __( 'Image ID.', 'litchi' ),
                                'type'        => 'integer',
                                'context'     => array( 'view', 'edit' ),
                            ),
                            'date_created'      => array(
                                'description' => __( "The date the image was created, in the site's timezone.", 'litchi' ),
                                'type'        => 'date-time',
                                'context'     => array( 'view', 'edit' ),
                                'readonly'    => true,
                            ),
                            'date_created_gmt'  => array(
                                'description' => __( 'The date the image was created, as GMT.', 'litchi' ),
                                'type'        => 'date-time',
                                'context'     => array( 'view', 'edit' ),
                                'readonly'    => true,
                            ),
                            'date_modified'     => array(
                                'description' => __( "The date the image was last modified, in the site's timezone.", 'litchi' ),
                                'type'        => 'date-time',
                                'context'     => array( 'view', 'edit' ),
                                'readonly'    => true,
                            ),
                            'date_modified_gmt' => array(
                                'description' => __( 'The date the image was last modified, as GMT.', 'litchi' ),
                                'type'        => 'date-time',
                                'context'     => array( 'view', 'edit' ),
                                'readonly'    => true,
                            ),
                            'src'               => array(
                                'description' => __( 'Image URL.', 'litchi' ),
                                'type'        => 'string',
                                'format'      => 'uri',
                                'context'     => array( 'view', 'edit' ),
                            ),
                            'name'              => array(
                                'description' => __( 'Image name.', 'litchi' ),
                                'type'        => 'string',
                                'context'     => array( 'view', 'edit' ),
                            ),
                            'alt'               => array(
                                'description' => __( 'Image alternative text.', 'litchi' ),
                                'type'        => 'string',
                                'context'     => array( 'view', 'edit' ),
                            ),
                            'position'          => array(
                                'description' => __( 'Image position. 0 means that the image is featured.', 'litchi' ),
                                'type'        => 'integer',
                                'context'     => array( 'view', 'edit' ),
                            ),
                        ),
                ...
                'attributes'            => array(
                    'description' => __( 'List of attributes.', 'litchi' ),
                    'type'        => 'array',
                    'context'     => array( 'view', 'edit' ),
                    'items'       => array(
                        'type'       => 'object',
                        'properties' => array(
                            'id'        => array(
                                'description' => __( 'Attribute ID.', 'litchi' ),
                                'type'        => 'integer',
                                'context'     => array( 'view', 'edit' ),
                            ),
                            'name'      => array(
                                'description' => __( 'Attribute name.', 'litchi' ),
                                'type'        => 'string',
                                'context'     => array( 'view', 'edit' ),
                            ),
                            'position'  => array(
                                'description' => __( 'Attribute position.', 'litchi' ),
                                'type'        => 'integer',
                                'context'     => array( 'view', 'edit' ),
                            ),
                            'visible'   => array(
                                'description' => __( "Define if the attribute is visible on the \"Additional information\" tab in the product's page.", 'litchi' ),
                                'type'        => 'boolean',
                                'default'     => false,
                                'context'     => array( 'view', 'edit' ),
                            ),
                            'variation' => array(
                                'description' => __( 'Define if the attribute can be used as variation.', 'litchi' ),
                                'type'        => 'boolean',
                                'default'     => false,
                                'context'     => array( 'view', 'edit' ),
                            ),
                            'options'   => array(
                                'description' => __( 'List of available term names of the attribute.', 'litchi' ),
                                'type'        => 'array',
                                'context'     => array( 'view', 'edit' ),
                                'items'       => array(
                                    'type' => 'string',
                ...
                'default_attributes'    => array(
                    'description' => __( 'Defaults variation attributes.', 'litchi' ),
                    'type'        => 'array',
                    'context'     => array( 'view', 'edit' ),
                    'items'       => array(
                        'type'       => 'object',
                        'properties' => array(
                            ...
                            'option' => array(
                                'description' => __( 'Selected attribute term name.', 'litchi' ),
                                'type'        => 'string',
                                'context'     => array( 'view', 'edit' ),
                            ),
                        ),
                ...
                'variations'            => array(
                    'description' => __( 'List of variations IDs.', 'litchi' ),
                    'type'        => 'array',
                    'context'     => array( 'view', 'edit' ),
                    'items'       => array(
                        'type' => 'integer',
                ...
                'grouped_products'      => array(
                    'description' => __( 'List of grouped products ID.', 'litchi' ),
                    'type'        => 'array',
                    'items'       => array(
                        'type' => 'integer',
                    ),
                ...
                'menu_order'            => array(
                    'description' => __( 'Menu order, used to custom sort products.', 'litchi' ),
                    'type'        => 'integer',
                    'context'     => array( 'view', 'edit' ),
                ),
                'meta_data'             => array(
                    'description' => __( 'Meta data.', 'litchi' ),
                    'type'        => 'array',
                    'context'     => array( 'view', 'edit' ),
                    'items'       => array(
                        'type'       => 'object',
                        'properties' => array(
                            'id'    => array(
                                'description' => __( 'Meta ID.', 'litchi' ),
                                'type'        => 'integer',
                                'context'     => array( 'view', 'edit' ),
                                'readonly'    => true,
                            ),
                            'key'   => array(
                                'description' => __( 'Meta key.', 'litchi' ),
                                'type'        => 'string',
                                'context'     => array( 'view', 'edit' ),
                            ),
                            'value' => array(
                                'description' => __( 'Meta value.', 'litchi' ),
                                'type'        => 'mixed',
                                'context'     => array( 'view', 'edit' ),
                            ),
                        ),
